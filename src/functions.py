...
def get_dataset() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame,
    ...
    # Remplacez par les noms de vos colonnes cibles
    X_train = train_set["comment_text"]
    y_train = train_set.drop(["id", "comment_text"], axis=1)

    # Jointure sur la colonne "ID"
    test_set = pd.merge(pd.read_csv("data/test.csv"),
                        pd.read_csv("data/test_labels.csv"), on='id')
...
def classif_result(model, y_train, y_test, X_svd, X_test_svd):
    ...
    y_test_pred_proba = np.array([class_prob[:, 1]
                                  for class_prob in y_test_pred_proba]).T
    ...
    for i in range(6):
        fpr_test, tpr_test, thresholds_test = roc_curve(
            y_test.iloc[:, i], y_test_pred_proba[:, i])
        plt.plot(fpr_test, tpr_test,
                 label=f'Courbe ROC de la classe {i} (Test)')
